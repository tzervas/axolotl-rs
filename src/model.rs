...
impl LoadedModel {
    ...
    ///
    /// Per-layer adapter (LoRA/QLoRA) injection is handled internally by the custom
    /// model architectures (`LoraLlama` / `QLoraLlama`) when loaded with PEFT/QLoRA features.
    /// For models loaded via `forward_with_adapters`, this delegates directly to the underlying
...
mod tests {
    ...
    fn test_loaded_model_forward_with_adapters() {
        ...
        let loaded = load_model(&config, &Device::Cpu).expect("load_model must succeed");
        let input_ids = Tensor::from_slice(&[1u32, 2, 3], (1, 3), &Device::Cpu).unwrap();
        let res = loaded.forward_with_adapters(&input_ids);
        assert!(res.is_ok());
    }
