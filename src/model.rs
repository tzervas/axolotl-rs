...
impl LoadedModel {
    ...
    ///
    /// Dispatches directly to the loaded model architecture (`LoraLlama`, `QLoraLlama`,
    /// or base LLaMA model). Per-layer adapter (LoRA/QLoRA) injection is handled
    /// internally by `LoraLlama` / `QLoraLlama` forward implementations.
    ///
    /// # Errors
    ///
    /// Returns an error if the forward pass fails.
    pub fn forward_with_adapters(&self, input_ids: &Tensor) -> Result<Tensor> {
        ...
        tracing::trace!("Forward pass complete (adapters evaluated in model architecture)");
