...
| `RmsNormWrapper` | ones-init CPU RMS | `unsloth_rs::kernels::RmsNorm` on every device (not CUDA-only) |

`apply_rotary_emb_ids` (packed positions) is feature-gated.

## Not in this pass

- Replacing `candle_nn::RmsNorm` inside candle-transformers LLaMA (weight sharing).
