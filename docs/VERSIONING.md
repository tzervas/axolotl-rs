...
`axolotl-rs` is **published on crates.io**, which makes it one of a small number of repos in this
...
A dependent on `axolotl-rs = "1.2"` accepts anything `>=1.2.0, <2.0.0`. Shipping a
...
Consumers pin the **major**: `axolotl-rs = "1.2"` tracks every compatible release up to but
excluding `2.0.0`. Pinning an exact patch (`"1.2.0"`) in a dependency example is a mistake —
...
| | Version |
| --- | --- |
| `Cargo.toml` in this repo | `1.2.0` |
| Newest git tag | `v1.2.0` |
| **crates.io `axolotl-rs`** | **`1.1.1`** |

`1.2.0` is tagged but **has never been published to crates.io**. Anyone reading the tag list
would reasonably conclude it shipped. It did not — `cargo add axolotl-rs` still resolves `1.1.1`.
